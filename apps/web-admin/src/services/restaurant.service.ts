...
export const restaurantApi = {
  getMyRestaurant: () =>
    apiClient.get('/restaurants/owner/mine').then((r) => r.data.data),

  getMenu: (restaurantId: string) =>
    apiClient.get(`/restaurants/${restaurantId}/menu`).then((r) => r.data.data),

  getOrders: (restaurantId: string, page = 1, status?: string) =>
    apiClient.get(`/orders/restaurant/${restaurantId}`, { params: { page, status } }).then((r) => r.data),
  ...
  getCategories: () =>
    apiClient.get('/menu/categories').then((r) => r.data.data),
