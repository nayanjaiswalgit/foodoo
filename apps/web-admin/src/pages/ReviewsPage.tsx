      ...
      {isLoading ? (
        ...
        <div className="space-y-4">
          {reviews.map((review: Record<string, unknown>) => (
            <div key={review._id as string} className="bg-white rounded-xl border border-gray-100 p-6">
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-medium text-sm">{(review.user as Record<string, string>)?.name ?? 'Anonymous'}</p>
